...
def ParseInputOptions():
	descriptions = {
		...
		'apikey': 'Your API key for Last.fm. It is recommended to put your API key in apikey.txt instead of passing it as an argument. An API key is required for this program to work',
		'output': 'The filename of the output image. The file extension doesn\t have to be included. Default: collage',
		'filetype': 'The filetype of the output image. Default: jpg',
		'jpeg_quality': 'The quality of the output image (when using JPEG). Higher values look better but produce a larger file size. Default: 100',
		'png_compression': 'The compression level of the output image (when using PNG). Higher values take longer to process but produce a smaller file size. Default: 9',
		'period': 'The period from which to fetch favorite albums. Default: forever',
		'layout': 'The layout of albums in the output image. Spiral places the top albums in the center and less popular albums near the edges using a spiral pattern. Topleft places images column by column, starting from the top-left corner. Default: topleft',
		'update_images': 'Whether to fetch updated albums and images from the server. Auto only updates the images if settings are changed or if the downloaded images are over a week old. Default: auto',
	...
	parser.add_argument('--jpeg_quality', '-q', help=descriptions['jpeg_quality'], type=int, default=100, metavar='{1-100}')
	parser.add_argument('--png_compression', '-c', help=descriptions['png_compression'], type=int, default=9, metavar='{0-9}')
	parser.add_argument('--period', '-p', help=descriptions['period'], choices=['forever', 'year', '6month', '3month', 'month', 'week'], default='forever')
	parser.add_argument('--layout', '-l', help=descriptions['layout'], choices=['spiral', 'topleft'], default='topleft')
	parser.add_argument('--update_images', '-u', help=descriptions['update_images'], choices=['auto', 'yes', 'no'], default='auto')
	...
	for i in range(len(dim)):
		if dim[i] > 9999:
			if dim[i] > 99999:
				raise ValueError('The ' + dim_labels[i] + ' entered is way too big. If you\'re serious about this you can modify the code to bypass this')
	...
	if args.apikey:
		...
		if os.path.exists('apikey.txt'):
			...
			raise Exception('apikey.txt does not exist. Please put your Last.fm key in apikey.txt or pass it as a command line argument')
...
def CreateCollage(dimensions, rows, cols, image_size, layout, images_required, output_filename, output_filetype, jpeg_quality, png_compression, ignore_warnings):
	...
	new_im.save(output_filename, output_filetype, quality=jpeg_quality, compress_level=png_compression, subsampling=0, exif=GenerateExif())
		...
		if os.path.exists('images/meta.txt'):
			try:
				with open('images/meta.txt') as file:
					...
					if data['user'] != user or data['period'] != period or data['images_fetched'] < images_required or data['time'] + 604800 <= start_time:
						return True
...
def main():
	...
	if CheckMeta(args.update_images, start_time, user, period, images_required):
		...
		while images_queue > 0:
			response = requests.get('http://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&format=json', {'user': user, 'api_key': api_key, 'period': period, 'limit': page_size, 'page': page})
		...
		while (image_counter < images_required and i < images_fetched):
			...
			if image_url:
				...
				local_file = open('images/' + str(image_counter).zfill(5) + '.' + extension, 'wb') # Open a local file with write binary permission
	...
	CreateCollage(dimensions, rows, cols, image_size, layout, images_required, output_filename, output_filetype, jpeg_quality, png_compression, ignore_warnings)
